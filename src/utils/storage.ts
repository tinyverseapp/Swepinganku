...
export function parseDateSafely(dateStr?: string | null): Date {
  ...
  if (parts.length === 3) {
    if (parts[0].length === 4) {
      ...
      if (!isNaN(y) && !isNaN(m) && !isNaN(d) && y > 1900 && m >= 0 && m <= 11 && d >= 1 && d <= 31) return new Date(y, m, d);
...
export function formatDateDMY(input?: string | Date | null): string {
  if (!input) return '';
  ...
  const dateObj = input instanceof Date ? input : parseDateSafely(input);
...
export function formatDpjpRaberCo(p: { dpjp?: string; doctorRole?: string; supervisingDpjp?: string }): string {
  const doc = p.dpjp || '-';
  const role = p.doctorRole || 'DPJP';
  const sup = p.supervisingDpjp;
  if (role === 'RABER') {
    return sup ? `DPJP: ${sup} / Raber: ${doc}` : `${doc} (Raber)`;
  }
  if (role === 'KONSUL') {
    return sup ? `DPJP: ${sup} / Konsul: ${doc}` : `${doc} (Konsul)`;
  }
  if (sup) {
    return `DPJP: ${doc} / Co: ${sup}`;
  }
  return doc;
}

export function calculateAgeFromDob(dobStr: string, refDate: string = today()): string {
  if (!dobStr || !dobStr.trim()) return '';
  const birth = parseDateSafely(dobStr.trim());
  const ref = parseDateSafely(refDate);
...
export function getWeeklyPatientList(startDate:string,endDate:string,division:string,teamCode?:string):Patient[]{const dates=getDatesBetween(startDate,endDate);const patientMap=new Map<string,Patient>();dates.forEach(dt=>loadPatients(dt,division,teamCode).forEach(p=>{if(!p.rm||isRemovedDoctor(p?.dpjp))return;const key=normalizeRmKey(p.rm);if(!key)return;const existing=patientMap.get(key);if(!existing)patientMap.set(key,{...p,date:dt,division:p.division||division});else patientMap.set(key,{...existing,name:p.name||existing.name,jk:p.jk||existing.jk,dob:p.dob||existing.dob,age:p.age||existing.age,dx:p.dx||existing.dx,dpjp:p.dpjp||existing.dpjp,room:p.room||existing.room,kamar:p.kamar||existing.kamar,lastDate:dt});}));return Array.from(patientMap.values()).sort((a,b)=>(a.room||'').localeCompare(b.room||'')||(a.name||'').localeCompare(b.name||''));}
export function computeWeeklyRecap(startDate:string,endDate:string,division:string,teamCode?:string):{rows:WeeklyRow[];dates:string[]}{const dates=getDatesBetween(startDate,endDate);const map=new Map<string,WeeklyRow>();dates.forEach(dt=>loadPatients(dt,division,teamCode).forEach(p=>{if(!p.rm||isRemovedDoctor(p?.dpjp))return;const key=normalizeRmKey(p.rm);if(!key)return;let r=map.get(key);if(!r){r={rm:p.rm,name:formatPatientNameWithHonorific(p.name,p.age,p.jk),jk:p.jk||'-',dob:p.dob||'',age:p.age||'-',dpjp:p.dpjp,dx:p.dx||'',first:dt,last:dt,lastRoom:p.room,lastKamar:p.kamar||'',days:{}};map.set(key,r);}r.name=formatPatientNameWithHonorific(p.name,p.age,p.jk);r.jk=p.jk;if(p.dob&&!r.dob)r.dob=p.dob;r.age=p.age;r.dpjp=p.dpjp;r.dx=p.dx;r.last=dt;r.lastRoom=normalizeRoomName(p.room);r.lastKamar=p.kamar;const bedOrKamar=formatKamarOrBed(p.room,p.kamar);r.days[dt]=`${normalizeRoomName(p.room)} (${bedOrKamar})`;}));return {rows:[...map.values()].sort((a,b)=>a.name.localeCompare(b.name)),dates};}
export function exportPatientsCSV(patients:Patient[],division:string='Bedah',dateOrRange:string='',filenamePrefix:string='daftar-pasien-sweeping',delimiter:string=';'):void{if(!patients.length)return;const seen=new Set<string>();const unique=patients.filter(p=>{const key=normalizeRmKey(p.rm)||p.id||p.name;if(seen.has(key))return false;seen.add(key);return true;});if(!unique.length)return;const headers=['No','No RM','Nama','Jenis Kelamin','DOB','Usia','Tgl Masuk','Diagnosis','DPJP/Raber/Co.','Ruangan','Status Pasien'];const rows=unique.map((p,i)=>{const cleanName=(p.name||'').replace(/^(tn\.?|ny\.?|an\.?|by\.?|nn\.?|sdr\.?|sdri\.?|tuan|nyonya|anak|bayi)\s+/i,'').trim();const cleanRm=(p.rm||'').replace(/["=]/g,'').trim();const rmVal=cleanRm?(cleanRm.startsWith('0')?`="${cleanRm}"`:cleanRm):'-';const cleanAge=(p.age||'').replace(/\s*(th|tahun|yr|year)s?/i,'').trim();const jkVal=p.jk?(p.jk.toUpperCase().startsWith('L')?'L':p.jk.toUpperCase().startsWith('P')?'P':p.jk):'-';const dobVal=p.dob?formatDateDMY(p.dob):'-';const admissionVal=p.weeklyStatus==='baru'?(p.admissionDate?formatDateDMY(p.admissionDate):(p.date?formatDateDMY(p.date):'-')):(p.admissionDate?formatDateDMY(p.admissionDate):'-');const doctorVal=formatDpjpRaberCo(p);const roomVal=formatRoomDisplay(p.room,p.kamar);const statusVal=p.weeklyStatus==='baru'?'Baru':p.weeklyStatus==='lama'?'Lama':'-';return [i+1,rmVal,cleanName||p.name||'-',jkVal,dobVal,cleanAge||p.age||'-',admissionVal,p.dx||'-',doctorVal,roomVal,statusVal];});triggerCsvDownload(buildExcelCsvString(headers,rows,delimiter),`${(filenamePrefix||'daftar-pasien-sweeping').replace(/\.csv$/,'')}.csv`);}
export function exportWeeklyCSV(rows:WeeklyRow[],dates:string[],division:string,startDate:string,endDate:string,delimiter:string=';'):void{if(!rows.length)return;const dayHeaders=dates.map(x=>{const d=parseDateSafely(x);try{return d.toLocaleDateString('id-ID',{weekday:'short',day:'2-digit',month:'2-digit'});}catch{return x;}});const headers=['No','No RM','Nama','Jenis Kelamin','DOB','Usia','Tgl Masuk','Diagnosis','DPJP/Raber/Co.','Ruangan Terakhir','Status Pasien',...dayHeaders];const dataRows=rows.map((r,i)=>{const cleanRm=(r.rm||'').replace(/["=]/g,'').trim();const rmVal=cleanRm?(cleanRm.startsWith('0')?`="${cleanRm}"`:cleanRm):'-';const dobVal=r.dob?formatDateDMY(r.dob):'-';const admissionVal=r.weeklyStatus==='baru'?(r.admissionDate?formatDateDMY(r.admissionDate):(r.first?formatDateDMY(r.first):'-')):'-';const jkVal=r.jk==='L'?'L':r.jk==='P'?'P':(r.jk||'-');const statusVal=r.weeklyStatus==='baru'?'Baru':r.weeklyStatus==='lama'?'Lama':'-';const roomDisplay=r.lastRoom?`${normalizeRoomName(r.lastRoom)}${r.lastKamar?` - ${formatKamarOrBed(r.lastRoom,r.lastKamar)}`:''}`:'-';return [i+1,rmVal,r.name,jkVal,dobVal,r.age||'-',admissionVal,r.dx||'-',r.dpjp||'-',roomDisplay,statusVal,...dates.map(d=>r.days[d]||'-')];});triggerCsvDownload(buildExcelCsvString(headers,dataRows,delimiter),`rekap-sweeping-matriks-${division.replace(/\s+/g,'-')}-${startDate}_${endDate}.csv`);}
...
export function findRotationWeekForDate(dateStr:string,roster:RotationWeek[]):RotationWeek|undefined{if(!dateStr||!roster?.length)return undefined;const exact=roster.find(w=>w.startDate&&w.endDate&&dateStr.trim()>=w.startDate.trim()&&dateStr.trim()<=w.endDate.trim());if(exact)return exact;try{const targetTime=new Date(dateStr).getTime();if(!isNaN(targetTime))return roster.find(w=>{if(!w.startDate||!w.endDate)return false;const s=new Date(w.startDate.trim()).getTime();const e=new Date(w.endDate.trim());e.setHours(23,59,59,999);return targetTime>=s&&targetTime<=e.getTime();});}catch{}return undefined;}
