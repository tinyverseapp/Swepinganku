...
export function exportPatientsCSV(patients:Patient[],division:string='Bedah',dateOrRange:string='',filenamePrefix:string='daftar-pasien-sweeping',delimiter:string=';'):void{if(!patients.length)return;const seen=new Set<string>();const unique=patients.filter(p=>{const key=normalizeRmKey(p.rm)||p.id||p.name;if(seen.has(key))return false;seen.add(key);return true;});if(!unique.length)return;const headers=['No','No. RM','Nama','Status Pasien','Tanggal Masuk','JK','Usia','Diagnosis','DPJP','Ruangan'];const rows=unique.map((p,i)=>{const cleanName=(p.name||'').replace(/^(tn\.?|ny\.?|an\.?|by\.?|nn\.?|sdr\.?|sdri\.?|tuan|nyonya|anak|bayi)\s+/i,'').trim();const cleanRm=(p.rm||'').replace(/["=]/g,'').trim();const rmVal=cleanRm?(cleanRm.startsWith('0')?`="${cleanRm}"`:cleanRm):'-';const cleanAge=(p.age||'').replace(/\s*(th|tahun|yr|year)s?/i,'').trim();const jkVal=p.jk?(p.jk.toUpperCase().startsWith('L')?'L':p.jk.toUpperCase().startsWith('P')?'P':p.jk):'-';const statusVal=p.weeklyStatus==='baru'?'Pasien Baru':p.weeklyStatus==='lama'?'Pasien Lama':'-';const admissionVal=p.weeklyStatus==='baru'?(p.admissionDate||p.date||'-'):'-';return [i+1,rmVal,cleanName||p.name||'-',statusVal,admissionVal,jkVal,cleanAge||p.age||'-',p.dx||'-',p.dpjp||'-',formatRoomDisplay(p.room,p.kamar)];});triggerCsvDownload(buildExcelCsvString(headers,rows,delimiter),`${(filenamePrefix||'daftar-pasien-sweeping').replace(/\.csv$/,'')}.csv`);}
export function exportWeeklyCSV(rows:WeeklyRow[],dates:string[],division:string,startDate:string,endDate:string,delimiter:string=';'):void{if(!rows.length)return;const dayHeaders=dates.map(x=>{const d=parseDateSafely(x);try{return d.toLocaleDateString('id-ID',{weekday:'short',day:'2-digit',month:'2-digit'});}catch{return x;}});const headers=['No','No. RM','Nama Pasien','Status Pasien','Tanggal Masuk','Jenis Kelamin','Usia','DPJP','Ruangan Terakhir','No. Kamar / Bed','Lokasi Lengkap','Diagnosis','Tgl Pertama Sweeping','Tgl Terakhir Sweeping',...dayHeaders];const dataRows=rows.map((r,i)=>{const statusVal=r.weeklyStatus==='baru'?'Pasien Baru':r.weeklyStatus==='lama'?'Pasien Lama':'-';const admissionVal=r.weeklyStatus==='baru'?(r.admissionDate||r.first||'-'):'-';return [i+1,r.rm?`="${r.rm.replace(/"/g,'')}"`:'-',r.name,statusVal,admissionVal,r.jk==='L'?'Laki-laki (L)':r.jk==='P'?'Perempuan (P)':r.jk,r.age,r.dpjp,normalizeRoomName(r.lastRoom),formatKamarOrBed(r.lastRoom,r.lastKamar),`${normalizeRoomName(r.lastRoom)} - ${formatKamarOrBed(r.lastRoom,r.lastKamar)}`,r.dx,r.first,r.last,...dates.map(d=>r.days[d]||'-')];});triggerCsvDownload(buildExcelCsvString(headers,dataRows,delimiter),`rekap-sweeping-matriks-${division.replace(/\s+/g,'-')}-${startDate}_${endDate}.csv`);}
