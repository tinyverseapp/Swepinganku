...
export function handoverPatientsLocal(teamCode:string,division:string,fromDate:string,toDate:string):{addedCount:number;patients:Patient[]}{const sourcePatients=loadPatients(fromDate,division,teamCode),targetPatients=loadPatients(toDate,division,teamCode);let addedCount=0;const merged=[...targetPatients];for(const sp of sourcePatients){if(!merged.some(tp=>tp.rm===sp.rm)){merged.push({...sp,id:`handover-${Date.now()}-${Math.random().toString(36).substr(2,6)}`,presenceStatus:'ada',updatedAt:new Date().toISOString()});addedCount++;}}savePatients(toDate,division,merged,teamCode);return {addedCount,patients:merged};}
