...
export function Topbar({ koasName, onUpdateKoasName, pageMode = 'dashboard', onPageModeChange, onOpenSettings }: TopbarProps) {
  ...
  const handleSaveProfile = async () => {
    ...
    setProfileError('');
    try {
      await updateProfile(user, { displayName: nextName, photoURL: nextPhoto || null });
      setUser({ ...user });
          ...
          {user && (
            <div ref={profileRef} className="relative shrink-0">
              <button type="button" onClick={() => isProfileOpen ? setIsProfileOpen(false) : openProfile()} className="group flex items-center gap-2 rounded-xl p-1 hover:bg-slate-100 transition-colors cursor-pointer" aria-label="Buka menu profil" aria-expanded={isProfileOpen}>
                ...
                <div className="hidden lg:block text-left max-w-32"><div className="text-xs font-bold text-slate-800 truncate">{displayName}</div><div className="text-[10px] text-slate-400 truncate">{user.email || 'Akun pengguna'}</div></div>
              </button>
              ...
              {isProfileOpen && <div className="absolute right-0 top-[calc(100%+8px)] w-[min(330px,calc(100vw-1.5rem))] rounded-2xl border border-slate-200 bg-white shadow-2xl overflow-hidden z-50">
                {!isEditingProfile ? <>
                  <div className="p-4 bg-gradient-to-br from-blue-50 to-white border-b border-slate-100"><div className="flex items-center gap-3"><div className="w-12 h-12 rounded-full overflow-hidden bg-blue-100 text-blue-700 flex items-center justify-center font-extrabold border-2 border-white ring-1 ring-blue-100 shrink-0">{user.photoURL ? <img src={user.photoURL} alt="Foto profil" className="w-full h-full object-cover" onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : initials}</div><div className="min-w-0"><div className="font-extrabold text-slate-900 truncate">{displayName}</div><div className="text-xs text-slate-500 truncate">{user.email}</div></div></div></div>
                  <div className="p-2"><button type="button" onClick={() => { setProfileName(user.displayName || koasName || ''); setProfilePhoto(user.photoURL || ''); setProfileError(''); setIsEditingProfile(true); }} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer"><Edit2 className="w-4 h-4 text-blue-600" />Edit Profil</button><button type="button" onClick={handleLogout} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold text-red-600 hover:bg-red-50 cursor-pointer"><LogOut className="w-4 h-4" />Keluar</button></div>
                </> : <div className="p-4">
