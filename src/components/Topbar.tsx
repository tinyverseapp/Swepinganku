...
export function Topbar({ koasName, onUpdateKoasName, pageMode = 'dashboard', onPageModeChange, onOpenSettings, activeTeam }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-xs">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 sm:py-0 sm:h-16 grid grid-cols-[1fr_auto] sm:flex sm:items-center gap-x-2.5 gap-y-2 sm:gap-3">
        ...
        {user && <div ref={profileRef} className="relative shrink-0 sm:order-2 self-center">
          <button type="button" onClick={() => isProfileOpen ? setIsProfileOpen(false) : openProfile()} className="group flex items-center gap-2 rounded-xl p-1 hover:bg-slate-100 transition-colors cursor-pointer" aria-label="Buka menu profil" aria-expanded={isProfileOpen}>
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-teal-100 text-teal-800 border-2 border-white ring-1 ring-teal-200 shadow-xs flex items-center justify-center font-extrabold text-xs sm:text-sm">{user.photoURL ? <img src={user.photoURL} alt="Foto profil" className="w-full h-full object-cover" onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : initials}</div>
          ...
          {isProfileOpen && <div className="absolute right-0 top-[calc(100%+8px)] w-[min(330px,calc(100vw-1.5rem))] rounded-2xl border border-slate-200 bg-white shadow-2xl overflow-hidden z-50">
            {!isEditingProfile ? <>
              <div className="p-4 bg-gradient-to-br from-teal-50/70 via-slate-50 to-white border-b border-slate-100">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-teal-100 text-teal-800 flex items-center justify-center font-extrabold border-2 border-white ring-1 ring-teal-200 shrink-0">
              ...
              <label className="block mb-3"><span className="text-xs font-semibold text-slate-600">Nama Profil</span><input type="text" value={profileName} onChange={(e) => setProfileName(e.target.value)} className="mt-1.5 w-full min-h-10 rounded-xl border border-slate-300 px-3 text-sm outline-none focus:border-teal-500 focus:ring-3 focus:ring-teal-100" placeholder="Nama lengkap" /></label>
              <label className="block mb-1"><span className="text-xs font-semibold text-slate-600">URL Foto Profil</span><input type="url" value={profilePhoto} onChange={(e) => setProfilePhoto(e.target.value)} className="mt-1.5 w-full min-h-10 rounded-xl border border-slate-300 px-3 text-sm outline-none focus:border-teal-500 focus:ring-3 focus:ring-teal-100" placeholder="https://..." /></label>
              <p className="text-[10px] text-slate-400 mb-4">Untuk akun Google, foto Google digunakan otomatis bila tersedia.</p>
              <div className="flex items-center gap-2"><button type="button" onClick={() => setIsEditingProfile(false)} className="flex-1 min-h-10 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 hover:bg-slate-50 cursor-pointer">Batal</button><button type="button" onClick={handleSaveProfile} disabled={profileBusy} className="flex-1 min-h-10 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold disabled:opacity-60 cursor-pointer">{profileBusy ? 'Menyimpan...' : 'Simpan Profil'}</button></div>
            </div>}
          </div>}
        </div>}

        {onPageModeChange && <div className="col-span-2 sm:col-span-1 sm:order-1 flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-semibold w-full sm:w-auto">
          <button type="button" onClick={() => onPageModeChange('dashboard')} className={`flex-1 sm:flex-initial flex items-center justify-center gap-1.5 py-2 sm:py-1.5 px-3 rounded-lg transition-all cursor-pointer min-h-[36px] ${pageMode === 'dashboard' ? 'bg-white text-teal-700 shadow-xs font-bold' : 'text-slate-600 hover:text-slate-900 active:bg-slate-200/60'}`}><LayoutGrid className="w-3.5 h-3.5 shrink-0" /><span>Kartu Pasien</span></button>
          <button type="button" onClick={() => onPageModeChange('document')} className={`flex-1 sm:flex-initial flex items-center justify-center gap-1.5 py-2 sm:py-1.5 px-3 rounded-lg transition-all cursor-pointer min-h-[36px] ${pageMode === 'document' ? 'bg-white text-teal-700 shadow-xs font-bold' : 'text-slate-600 hover:text-slate-900 active:bg-slate-200/60'}`}><FileText className="w-3.5 h-3.5 shrink-0" /><span>Dokumen Sweeping</span></button>
        </div>}
      </div>
    </header>
  );
