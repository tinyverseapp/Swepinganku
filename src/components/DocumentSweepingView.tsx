...
export function DocumentSweepingView({ patients, date, division, koasName, dpjps, allRooms, onDateChange, onBackToDashboard, onAddPatient, onEditPatient, activeTeam, onOpenTeamModal, onHandoverPatients, onOpenAiImport }: DocumentSweepingViewProps) {
  ...
  const handleCopyWA = async (withEmptyRooms: boolean = true) => {
    ...
    try {
      ...
      setCopied(true);
      setCopyFormatFeedback(withEmptyRooms ? 'all' : 'filled');
      setIsCopyDropdownOpen(false);
      setTimeout(() => {
        setCopied(false);
        setCopyFormatFeedback(null);
      }, 2500);
    } catch {
  ...
  return <div className="min-h-screen bg-[#f8f9fa] flex flex-col text-slate-800 font-sans">
    <header className="bg-white border-b border-slate-200 px-3 sm:px-4 py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sticky top-0 z-20 shadow-xs print:hidden">
      ...
      <div className="flex items-center gap-1.5 overflow-visible flex-wrap sm:flex-nowrap">
        {/* Dropdown Menu Salin WA */}
        <div className="relative shrink-0" ref={copyDropdownRef}>
          <div className="inline-flex rounded-lg shadow-2xs overflow-hidden border border-emerald-700/20 bg-emerald-600 hover:bg-emerald-700 transition-colors">
            <button
              ...
              aria-haspopup="true"
            >
              {copied ? <Check className="w-4 h-4 text-emerald-100 shrink-0" /> : <Copy className="w-4 h-4 shrink-0" />}
              <span>
                {copied
                  ? copyFormatFeedback === 'filled'
                    ? 'Tersalin (Ruangan Berisi)!'
                    : 'Tersalin (Semua Ruangan)!'
                  : 'Salin WA'}
              </span>
            </button>
            <button
              type="button"
              onClick={() => setIsCopyDropdownOpen((prev) => !prev)}
              className="px-2 py-1.5 text-white/90 hover:text-white border-l border-emerald-500/60 hover:bg-emerald-700 min-h-[36px] flex items-center justify-center transition-colors cursor-pointer"
              title="Pilihan format Salin WA"
              aria-label="Pilihan format Salin WA"
              aria-expanded={isCopyDropdownOpen}
            >
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isCopyDropdownOpen ? 'rotate-180' : ''}`} />
            </button>
          </div>
          ...
          {isCopyDropdownOpen && (
            <div className="absolute left-0 mt-1.5 w-76 sm:w-80 bg-white rounded-xl shadow-xl border border-slate-200 p-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-bold text-slate-800 group-hover:text-emerald-800 flex items-center justify-between">
                    <span>Salin Hanya Ruangan Berisi Pasien</span>
