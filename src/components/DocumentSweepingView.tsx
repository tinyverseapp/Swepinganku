  ...
  return <div className="min-h-screen bg-[#f8f9fa] flex flex-col text-slate-800 font-sans">
    <header className="bg-white border-b border-slate-200 px-3 sm:px-4 py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sticky top-0 z-20 shadow-xs print:hidden">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto">
        ...
        <button onClick={() => setIncludeEmptyRooms(!includeEmptyRooms)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-blue-50 border border-blue-200 min-h-[36px]"><SlidersHorizontal className="w-3.5 h-3.5" />R. Kosong (0): {includeEmptyRooms ? 'On' : 'Off'}</button>
    ...
    <div className="bg-white border-b border-slate-200 px-4 py-2 flex items-center justify-between gap-3 overflow-x-auto print:hidden sticky top-[53px] z-10">
      <div className="flex items-center gap-1.5"><span className="text-xs font-bold text-slate-600 flex items-center gap-1.5 mr-1"><CalendarDays className="w-4 h-4 text-blue-600" />Tab Hari:</span><button onClick={handlePrevDay} className="p-1.5 border border-slate-200 rounded-lg"><ChevronLeft className="w-4 h-4" /></button>{weekDays.map(w => <button key={w.date} onClick={() => onDateChange?.(w.date)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold border ${w.date === date ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>{w.dayName} {w.dayOfMonth}</button>)}<button onClick={handleNextDay} className="p-1.5 border border-slate-200 rounded-lg"><ChevronRight className="w-4 h-4" /></button>{!isCurrentDateToday && <button onClick={handleToday} className="px-2.5 py-1.5 text-xs font-bold text-blue-700 bg-blue-50 border border-blue-200 rounded-lg">Hari Ini</button>}</div>
    ...
    <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
      {isSidebarOpen && <aside className="relative fixed inset-y-0 left-0 z-50 md:static md:z-auto w-72 lg:w-80 bg-white border-r border-slate-200 flex-shrink-0 flex flex-col shadow-xl md:shadow-xs print:hidden">
        ...
        <div className="p-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50"><div className="flex items-center gap-2"><FileText className="w-4 h-4 text-blue-600" /><span className="font-bold text-xs uppercase tracking-wider">Tab Dokumen</span></div><button type="button" onClick={() => setIsSidebarOpen(false)} className="w-8 h-8 border border-slate-200 rounded-xl"><PanelLeftClose className="w-4 h-4" /></button></div>
        <div className="p-2 space-y-1 overflow-y-auto flex-1">
          <button type="button" onClick={() => setActiveTab('all')} className={`w-full text-left px-3 py-2.5 rounded-xl text-xs font-semibold flex items-center justify-between ${activeTab === 'all' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'hover:bg-slate-100'}`}><span>Semua Pasien</span><span>{patients.length}</span></button>
          <div className="pt-2 pb-1 px-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">Dokter ({dpjps.length})</div>
          {dpjps.map(dpjp => { const count = patients.filter(p => p.dpjp === dpjp).length; const roles = getDoctorRoles(dpjp); return <button key={dpjp} type="button" onClick={() => setActiveTab(dpjp)} className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-between ${activeTab === dpjp ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'hover:bg-slate-100'}`}><div className="flex items-center gap-2 truncate"><span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-slate-200 shrink-0">{getDpjpInitials(dpjp)}</span><span className="truncate" title={dpjp}>{dpjp}</span><span className="flex gap-1 shrink-0">{(roles.length ? roles : ['DPJP']).map(role => <span key={role} className="text-[8px] uppercase font-bold bg-slate-100 border border-slate-200 rounded px-1 py-0.5">{role}</span>)}</span></div><span>{count}</span></button>; })}
        </div>
      </aside>}
      ...
      <main className="flex-1 overflow-y-auto p-2 sm:p-6 lg:p-10 flex justify-center">
        <div className="bg-white w-full max-w-4xl shadow-md border border-slate-200/80 rounded-sm p-4 sm:p-10 md:p-16 min-h-[900px] text-slate-900 text-sm sm:text-[15px] leading-relaxed relative print:shadow-none print:border-none print:p-0">
          ...
          <div className="my-5 space-y-1 font-bold"><p>*{dayName}, <span className="text-rose-600">{fullDateStr}</span>*</p><p>*Total pasien: <span className="text-rose-600">{currentTabPatients.length} pasien</span>*</p>{activeTab !== 'all' && <p className="text-blue-700">*{getDoctorRoles(activeTab).join(' / ') || 'DPJP'}: {activeTab}*</p>}</div>
