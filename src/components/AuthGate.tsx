...
export function AuthGate({ children }: AuthGateProps) {
  ...
  if (!user) {
    ...
    return (
      <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-teal-50/40 flex items-center justify-center px-4 py-8">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden">
            ...
            <div className="px-6 sm:px-8 pb-8">
              <div className="grid grid-cols-2 p-1 mb-5 rounded-xl bg-slate-100 border border-slate-200">
                <button type="button" onClick={() => switchMode('login')} className={`min-h-10 rounded-lg text-sm font-bold flex items-center justify-center gap-1.5 ${!isRegister ? 'bg-white text-teal-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><LogIn className="w-4 h-4" />Login</button>
                <button type="button" onClick={() => switchMode('register')} className={`min-h-10 rounded-lg text-sm font-bold flex items-center justify-center gap-1.5 ${isRegister ? 'bg-white text-teal-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><UserPlus className="w-4 h-4" />Daftar Akun</button>
              </div>
              ...
              <form onSubmit={handleEmailAuth} className="space-y-3.5">
                ...
                <button type="submit" disabled={busy} className="w-full min-h-11 rounded-xl bg-teal-700 hover:bg-teal-800 text-white font-bold flex items-center justify-center gap-2 disabled:opacity-60 transition-colors">{isRegister ? <UserPlus className="w-4 h-4" /> : <LogIn className="w-4 h-4" />}{busy ? 'Memproses...' : isRegister ? 'Buat Akun' : 'Login'}</button>
              </form>
