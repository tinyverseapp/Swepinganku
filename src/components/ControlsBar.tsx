...
export function ControlsBar({
  ...
  // The search field visually belongs to the patient-card header, which is
  // rendered after this controls component. Use document coordinates once per
  // layout change and render it as an absolute element. Unlike position:fixed,
  // it moves with the document naturally during scrolling, so there is no
  // scroll-driven coordinate update and no jitter.
  useEffect(() => {
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-3.5 sm:p-5 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
          <div>
            ...
            <input type="date" value={date} onChange={(e) => onDateChange(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2.5 sm:py-2 text-sm sm:text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all cursor-pointer min-h-[42px]" />
          </div>
          <div>
            ...
            <select value={effectiveDivision} onChange={(e) => onDivisionChange(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2.5 sm:py-2 text-sm sm:text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all cursor-pointer min-h-[42px]">
          ...
          <div className="sm:col-span-2 lg:col-span-2 md:hidden">
            ...
            <div className="relative">
              ...
              <input type="text" value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} placeholder="Ketik nama pasien / no RM / diagnosis..." className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-9 pr-3.5 py-2.5 sm:py-2 text-sm sm:text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all min-h-[42px]" />
            </div>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {onOpenAiImport && <button type="button" onClick={onOpenAiImport} className="group flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3.5 py-2.5 sm:py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-xs transition-all cursor-pointer min-h-[40px]" title="Salin & Tempel Catatan Teks untuk Diubah Menjadi Pasien Otomatis oleh AI"><AiSparkleIcon size={16} className="shrink-0 transition-transform duration-200 group-hover:scale-115" /><span>AI Impor Catatan</span></button>}
