...
import { getJoinedDivisions } from '../utils/teamRegistry';
...
export function ControlsBar({
  ...
  const visibleDivisions = (divisions || []).filter((d) => joinedDivisions.includes(d) || d === division);
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-3.5 sm:p-5 shadow-xs space-y-4">
      {activeTeam && onOpenTeamModal && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 rounded-xl bg-gradient-to-r from-blue-50/90 via-indigo-50/60 to-slate-50 border border-blue-200/80 shadow-2xs">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs font-bold text-slate-900">{activeTeam.teamName || activeTeam.division}</span>
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap sm:flex-nowrap w-full sm:w-auto">
            ...
            <button type="button" onClick={onOpenTeamModal} className="flex-1 sm:flex-initial px-3 py-2 text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors flex items-center justify-center gap-1 shadow-2xs cursor-pointer min-h-[38px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
        ...
        <div>
          ...
          <select value={division} onChange={(e) => onDivisionChange(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2.5 sm:py-2 text-sm sm:text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer min-h-[42px]">
        ...
        <div className="sm:col-span-2 lg:col-span-2">
          ...
          <div className="relative">
            ...
            <input type="text" value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} placeholder="Ketik nama pasien / no RM / diagnosis..." className="w-full bg-slate-50 border border-slate-300 rounded-xl pl-9 pr-3.5 py-2.5 sm:py-2 text-sm sm:text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all min-h-[42px]" />
          </div>
        </div>
      </div>
      ...
      <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {onAddPatient && (
            <button type="button" onClick={onAddPatient} className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-4 py-2.5 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-xs transition-colors cursor-pointer min-h-[40px]" title="Tambah Pasien Baru">
              <UserPlus className="w-4 h-4 shrink-0" /><span>+ Tambah Pasien</span>
            </button>
          )}
          <button type="button" onClick={onOpenWeekly} className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3.5 py-2.5 sm:py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold text-xs rounded-xl border border-slate-200 transition-colors shadow-2xs cursor-pointer min-h-[40px]" title="Lihat Rekapitulasi Mingguan dan Ekspor Excel/CSV">
            <CalendarRange className="w-3.5 h-3.5 text-slate-600 shrink-0" /><span>Rekap Mingguan &amp; CSV</span>
          </button>
        </div>
      </div>
    </div>
