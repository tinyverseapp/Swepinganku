      ...
      {open && (
        ...
        >
          ...
          <div className="max-h-64 overflow-y-auto p-2">
            {error ? <div className="flex items-center gap-2 px-3 py-3 text-xs text-amber-700"><RefreshCw className="w-3.5 h-3.5" />{error}</div> : members.length === 0 ? <div className="py-5 text-xs text-slate-400 text-center">Belum ada akun lain di tim ini.</div> : <div className="space-y-0.5">{members.map((member) => <div key={member.uid} className="flex items-center gap-2.5 rounded-xl px-2.5 py-2 hover:bg-slate-50"><div className="w-8 h-8 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center text-[10px] font-extrabold shrink-0">{(member.name || member.email || '?').trim().charAt(0).toUpperCase()}</div><div className="min-w-0 flex-1"><div className="text-xs font-bold text-slate-800 truncate">{member.name || 'Pengguna'}</div><div className="text-[10px] text-slate-500 truncate">{member.email || 'Email tidak tersedia'}</div></div><Circle className="w-2.5 h-2.5 fill-emerald-500 text-emerald-500 shrink-0" title="Terdaftar di tim" /></div>)}</div>}
          </div>
