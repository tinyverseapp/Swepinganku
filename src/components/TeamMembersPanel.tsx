  ...
  return (
    <div ref={panelRef} className="fixed left-3 bottom-3 z-50">
      <button
        ...
        className="flex items-center gap-2 min-h-9 max-w-[220px] rounded-full border border-slate-200 bg-white/95 backdrop-blur shadow-lg hover:shadow-xl hover:bg-slate-50 px-3 py-2 transition-all cursor-pointer"
        ...
        <div className="absolute left-0 bottom-[calc(100%+8px)] w-[min(340px,calc(100vw-1.5rem))] rounded-2xl border border-slate-200 bg-white shadow-2xl overflow-hidden z-50">
          <div className="px-4 py-3 bg-gradient-to-br from-blue-50 to-white border-b border-slate-100">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0"><div className="text-sm font-extrabold text-slate-900 truncate">{teamName}</div><div className="text-[10px] text-slate-500 mt-0.5">Kode tim: {teamCode}</div></div>
              ...
              <div className="space-y-0.5">
                {members.map((member) => (
                  <div key={member.uid} className="flex items-center gap-2.5 rounded-xl px-2.5 py-2 hover:bg-slate-50">
                    <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-[10px] font-extrabold shrink-0">{(member.name || member.email || '?').trim().charAt(0).toUpperCase()}</div>
