  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col border border-slate-200 overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-blue-50/80 via-white to-indigo-50/60">
        <div className="flex items-center gap-3 min-w-0"><div className="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center shrink-0"><Sparkles className="w-5 h-5 text-amber-300" /></div><div className="min-w-0"><div className="flex items-center gap-2 flex-wrap"><h2 className="text-base sm:text-lg font-extrabold text-slate-900">AI Impor Catatan Pasien</h2><span className="px-2 py-0.5 rounded-md text-[11px] font-bold bg-blue-100 text-blue-800 border border-blue-200">{division}</span><span className="text-xs text-slate-500">{formatIndonesianDate(date)}</span></div><p className="text-xs text-slate-500 truncate">AI memahami hubungan DPJP utama, RABER (rawat bersama), dan KONSUL.</p></div></div>
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
        ...
        <div className="space-y-2"><div className="flex items-center justify-between gap-2 flex-wrap"><label className="text-xs font-bold text-slate-700 flex items-center gap-1.5"><FileText className="w-4 h-4 text-blue-600" />Tempel Catatan Teks Pasien:</label><div className="flex gap-1.5"><button type="button" onClick={handlePasteClipboard} className="px-2.5 py-1 rounded-lg bg-slate-100 text-xs font-semibold">Tempel</button><button type="button" onClick={() => { setInputText(sampleNote); setError(null); }} className="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 text-xs font-semibold">Contoh</button>{inputText && <button type="button" onClick={handleClear} className="px-2.5 py-1 rounded-lg bg-slate-100 text-xs font-semibold"><RotateCcw className="w-3.5 h-3.5 inline mr-1" />Bersihkan</button>}</div></div><textarea value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Tempel catatan operan pasien di sini..." className="w-full min-h-52 rounded-xl border border-slate-300 p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" /><button type="button" onClick={handleRunAi} disabled={isLoading || !inputText.trim()} className="w-full min-h-11 rounded-xl bg-blue-600 text-white font-bold text-sm disabled:opacity-50"><Sparkles className="w-4 h-4 inline mr-2" />{isLoading ? 'Menganalisis...' : 'Analisis dengan AI'}</button></div>
