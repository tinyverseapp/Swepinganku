...
import { getSavedActiveTeam, loadJoinedTeams } from './utils/teamRegistry';
import {
  subscribeToPatients,
  savePatientsBatch,
...
export default function App() {
  ...
  const handleDivisionChange = (nextDivision: string) => {
    ...
    saveCurrentTeam(joinedTeam);
    setDivision(joinedTeam.division);
    setSearchQuery('');
    setSelectedDpjpFilter('all');
  };
  ...
  const handleConfirmDelete = (patient: Patient) => {
    ...
    } else showToast(`Data ${patient.name} telah dihapus.`);
  };
  ...
  const handleQuickSharePatient = async (patient: Patient) => {
    ...
    try { await navigator.clipboard.writeText(text); showToast(`Data ${patient.name} berhasil disalin ke clipboard!`); }
    catch { showToast('Gagal menyalin otomatis ke clipboard.'); }
  };
  ...
  const handleCopyFromDay = (fromDate: string, dayName: string) => {
    ...
    if (!raw) { showToast(`Data pada hari ${dayName} (${fromDate}) masih kosong.`); return; }
    try {
      const sourcePatients: Patient[] = JSON.parse(raw);
      const currentRms = new Set(patients.map((p) => p.rm.trim().toLowerCase()));
      const toAdd = sourcePatients.filter((p) => !currentRms.has(p.rm.trim().toLowerCase())).map((p) => ({ ...p, id: `pt_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`, teamCode: activeTeam.teamCode, date, division, updatedAt: new Date().toISOString() }));
  ...
  const handleSwitchTeam = (newTeam: DivisionTeam, carryOverFromYesterday: boolean) => {
    ...
    if (carryOverFromYesterday) {
      ...
      setPatients(withFirebaseMeta);
      if (newTeam.teamCode && withFirebaseMeta.length) savePatientsBatch(newTeam.teamCode, date, withFirebaseMeta).catch((error) => showToast(`Firebase gagal menyimpan operan: ${error.message}`));
      showToast(`Terhubung ke Tim ${newTeam.teamName || newTeam.teamCode}. Dioper ${result.addedCount} pasien.`);
    } else showToast(`Terhubung ke Tim ${newTeam.teamName || newTeam.teamCode}.`);
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col selection:bg-blue-100 selection:text-blue-900">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-4 sm:space-y-5">
        {pageMode === 'document' ? (
          ...
          <>
            <ControlsBar date={date} division={division} divisions={DIVISIONS} onDateChange={handleDateChange} onDivisionChange={handleDivisionChange} searchQuery={searchQuery} onSearchChange={setSearchQuery} onOpenWeekly={() => setIsWeeklyModalOpen(true)} activeTeam={activeTeam} onOpenTeamModal={() => setIsTeamModalOpen(true)} onHandoverPatients={handleHandoverYesterday} onAddPatient={() => { setEditingPatient(null); setIsPatientModalOpen(true); }} />
