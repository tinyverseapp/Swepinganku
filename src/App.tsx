...
export default function App() {
  ...
  const [deleteAllDayTarget, setDeleteAllDayTarget] = useState<{ date: string; dayName?: string; count: number } | null>(null);
  ...
  const handleSavePatient = (patientData: Patient) => {
    const finalWeeklyStatus = patientData.weeklyStatus || 'baru';
    const finalAdmissionDate = finalWeeklyStatus === 'baru' ? (patientData.admissionDate || date) : '';
    const patient: Patient = {
      ...patientData,
      presenceStatus: patientData.presenceStatus || 'belum_periksa',
      weeklyStatus: finalWeeklyStatus,
  ...
  const handleToggleWeeklyStatus = (patient: Patient) => {
    const nextStatus: Patient['weeklyStatus'] = patient.weeklyStatus === 'lama' ? 'baru' : 'lama';
    const nextAdmission = nextStatus === 'baru' ? (patient.admissionDate || patient.date || date) : '';
    const updatedPatient: Patient = {
      ...patient,
      weeklyStatus: nextStatus,
      admissionDate: nextAdmission,
      updatedAt: new Date().toISOString(),
    };
    const updated = patients.map((p) => (p.id === patient.id ? updatedPatient : p));
    setPatients(updated);
    savePatients(date, division, updated, activeTeam.teamCode);
    if (activeTeam.teamCode) {
      upsertPatientToFirestore(updatedPatient, activeTeam.teamCode, date).catch(() => {});
    }
    showToast(`${patient.name} diubah menjadi ${nextStatus === 'baru' ? 'Pasien Baru' : 'Pasien Lama'}.`);
  };
  ...
  const handleConfirmDeleteAllDay = async (targetDate: string) => {
    if (!targetDate) return;
    if (targetDate === date) {
      setPatients([]);
    }
    clearPatientsForDate(targetDate, division, activeTeam.teamCode);

    if (activeTeam.teamCode) {
      try {
        await deleteAllPatientsForDateFromFirestore(activeTeam.teamCode, targetDate);
        showToast(`Semua pasien pada ${targetDate} berhasil dihapus dari Firebase.`);
      } catch (error: any) {
        showToast(`Data lokal dikosongkan, namun Firebase gagal: ${error?.message || 'kendala jaringan'}`);
      }
    } else {
      showToast(`Semua pasien pada ${targetDate} berhasil dihapus.`);
    }
  };
  ...
  const handleConfirmHandoverYesterday = async () => {
    if (!division || !activeTeam.teamCode) { showToast('Silakan bergabung ke tim terlebih dahulu.'); return; }
    ...
    if (result.addedCount > 0) {
      ...
      try {
        await savePatientsBatch(activeTeam.teamCode, date, withFirebaseMeta);
        showToast(`Berhasil mengoper ${result.addedCount} pasien dan sinkron ke Firebase.`);
      } catch (error: any) {
        showToast(`Operan tersimpan lokal, tetapi Firebase gagal: ${error.message}`);
      }
    } else {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col selection:bg-teal-100 selection:text-teal-900">
    ...
    <PatientModal isOpen={isPatientModalOpen} initialData={editingPatient} defaultDpjp={consultants[0] || ''} existingDpjps={existingDpjps} division={division} divisionConsultants={consultants} activeDate={date} onClose={() => { setIsPatientModalOpen(false); setEditingPatient(null); }} onSave={handleSavePatient} />
    ...
    <DeleteAllConfirmModal target={deleteAllDayTarget} division={division} onClose={() => setDeleteAllDayTarget(null)} onConfirm={handleConfirmDeleteAllDay} />
