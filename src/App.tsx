...
import { today, getKoasName, saveKoasName, getCompactMode, setCompactMode, loadPatients, savePatients, saveCurrentTeam, handoverYesterdayPatients, getStorageKey, isRemovedDoctor } from './utils/storage';
...
import { Users, UserPlus, GitPullRequest, CheckCircle2, Stethoscope, Settings, Minimize2, Maximize2 } from 'lucide-react';
...
export default function App() {
  ...
  const handleToggleCompactMode = useCallback(() => setCompactModeState((prev) => { const next = !prev; setCompactMode(next); showToast(next ? 'Mode Ringkas diaktifkan.' : 'Mode Ringkas dinonaktifkan.'); return next; }), [showToast]);
  useEffect(() => { if (!toastMessage) return; const timer = setTimeout(() => setToastMessage(null), 3200); return () => clearTimeout(timer); }, [toastMessage]);

  // The team document is shared in Firestore. A rename made by any member
  // immediately updates this account, Topbar, ControlsBar and the floating team pill.
  useEffect(() => {
