...
export default function App() {
  ...
  // A patient belongs to the surgical division even when the surgeon is RABER/KONSUL.
  // Keep every patient in the dashboard and expose both the role-doctor and the
  // supervising/main DPJP in the doctor filter. Selecting a surgeon therefore shows
  // their RABER/KONSUL patients without removing those patients from the main list.
  ...
  const existingDpjps = useMemo(() => {
    ...
    patients.forEach((p) => {
      if (p.dpjp?.trim() && !isRemovedDoctor(p.dpjp)) set.add(p.dpjp.trim());
      if (p.supervisingDpjp?.trim() && !isRemovedDoctor(p.supervisingDpjp)) set.add(p.supervisingDpjp.trim());
    });
    return Array.from(set);
  }, [consultants, patients]);

  const filteredPatients = useMemo(() => patients.filter((p) => {
    if (selectedDpjpFilter !== 'all') {
